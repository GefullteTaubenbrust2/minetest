...
vec4 sampleClouds(vec2 uv, float raw_depth) {
	...
	float auroraVisibility = 1.0;
	if (raw_depth < 1.0) auroraVisibility = 0.0;

	return vec4(
		mix(auroraDark, auroraBright, cloudsKey.b) * cloudsKey.b * auroraVisibility * auroraFactor + 
		cloudsKey.r * cloudColor * (darkColor * exp(-cloudsKey.g) + dayLight * sunTint * cloudsKey.g),
	cloudsKey.r);
}

vec4 getClouds(vec2 uv, float raw_depth) {
#if (VOLUMETRICS_UNDERSAMPLING <= 1)
		return sampleClouds(uv, raw_depth);
#else
	return
		sampleClouds(uv - texelSize0 * vec2(-1.0, -1.0), raw_depth) / 9.0 +
		sampleClouds(uv - texelSize0 * vec2( 0.0, -1.0), raw_depth) / 9.0 +
		sampleClouds(uv - texelSize0 * vec2( 1.0, -1.0), raw_depth) / 9.0 +
		sampleClouds(uv - texelSize0 * vec2(-1.0,  0.0), raw_depth) / 9.0 +
		sampleClouds(uv - texelSize0 * vec2( 0.0,  0.0), raw_depth) / 9.0 +
		sampleClouds(uv - texelSize0 * vec2( 1.0,  0.0), raw_depth) / 9.0 +
		sampleClouds(uv - texelSize0 * vec2(-1.0,  1.0), raw_depth) / 9.0 +
		sampleClouds(uv - texelSize0 * vec2( 0.0,  1.0), raw_depth) / 9.0 +
		sampleClouds(uv - texelSize0 * vec2( 1.0,  1.0), raw_depth) / 9.0;
...
void main(void)
{
	...
	float raw_depth = getDepth(screenspaceCoordinate);

	float depth = toLinearDepth(raw_depth) / normalize(viewDirection).z;
	...
	if (depth > minPlane) {
		vec4 finalColor = getClouds(screenspaceCoordinate * 0.5 + 0.5, raw_depth);
